...
public interface IAppPasswordStore
{
    ...
    /// <param name="kind">The app password kind.</param>
    /// <param name="ct">The cancellation token.</param>
    /// <returns>The plaintext token plus the stored password record.</returns>
    Task<(string Token, AppPassword Password)> CreateAsync(Guid userId, string label, string kind, CancellationToken ct = default);
    ...
    /// <param name="kind">Optional app password kind filter.</param>
    /// <param name="ct">The cancellation token.</param>
    /// <returns>The matching password record when valid; otherwise <c>null</c>.</returns>
    Task<AppPassword?> ValidateTokenAsync(string token, string? kind = null, CancellationToken ct = default);
}
